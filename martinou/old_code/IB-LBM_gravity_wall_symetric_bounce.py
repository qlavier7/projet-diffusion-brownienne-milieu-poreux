...
output_gif_name = "symetric_bounce_wall_without_lubrication" # name of the output gif file (only if live_flow_plot=True)
...
N_markers = len(D_particles) # number of particle markers - need to offset initial positions for anything to happen when increasing this from 1
...
init_marker_pos[0, 1] = cy_particle - 1.15*D_particle1
init_marker_pos[0, 2] = cz_particle

# marker 2
init_marker_pos[1, 0] = cx_particle + 2*D_particle2
init_marker_pos[1, 1] = cy_particle - 1.15*D_particle2
init_marker_pos[1, 2] = cz_particle

stop_dist = 0 # minimum distance from the particle to the wall before the simulation is stopped
...
#%% Solver Parameters
sim_time = 1200 # simulation time [s] - adjust accordingly
Nt = int(sim_time) # number of time steps (since dt=1)

outevery = int(Nt/N_outputs) # generate an output every this many steps
outevery = 10
lubrication_threshold = 0.67 # threshold distance for lubrication correction
...
def lubrication_correction_forcing(N_markers, marker_pos, marker_vel, marker_radii, 
                              lubrication_threshold, marker_f, wall_y, F_gravity):
    """
    ...
    for i in range(N_markers):
        h_wall = (marker_pos[i, 1] - wall_y) - marker_radii[i]  # gap between the particle surface and the wall
        
        if h_wall <= lubrication_threshold:
            h_safe = max(h_wall, 0.2*lubrication_threshold)  # avoid a singularity if the particle is (slightly) touching/overlapping
            R_eff = marker_radii[i]  # sphere-wall: the wall's "radius" -> infinity, so R_eff -> marker_radii[i]
            UR = marker_vel[i, 1]  # the wall is fixed, so the relative velocity along the normal is just marker_vel[i, 1]
            coef = -6 * np.pi * nu * R_eff**2 * (1/h_safe - 1/lubrication_threshold) * UR

            marker_f[i, 1] += coef  # force is directed along the wall normal (0, 1, 0)"""

@nb.jit(nopython=True, fastmath=True)
def resolve_particle_collisions(marker_pos, marker_vel, D_particles, rhos, wall_y):
    ...
    It doesn't work for more than two particles.
    
    It also resolves particle-wall collisions with the bottom wall located at
    y = wall_y (unit normal (0, 1, 0)). The wall is treated as an infinitely
    massive, fixed object, so the collision is a "slip" (specular) elastic
    bounce: only the wall-normal (y) velocity component is reversed, the
    tangential (x, z) components are left untouched.
    """
    ...
    for i in range(N_markers):
        ...
        if collided[i]:
            # This marker already had its position advanced this step as part of a
            # particle-particle collision above. Simply clamp/reflect it if that
            # move pushed it through the wall (approximate treatment of the rare
            # double-collision case within a single time step).
            if marker_pos[i, 1] - Ri < wall_y:
                marker_pos[i, 1] = wall_y + Ri
                if marker_vel[i, 1] < 0.0:
                    marker_vel[i, 1] = -marker_vel[i, 1]
            continue
        
        y0 = marker_pos[i, 1]
        vy = marker_vel[i, 1]
        g0 = y0 - Ri - wall_y  # gap between the particle surface and the wall at the start of the step
        
        if g0 < 0.0:
            # Particle is already touching/slightly overlapping the wall: snap it back
            # onto the wall surface and, if still moving into the wall, reflect it.
            marker_pos[i, 1] = wall_y + Ri
            y0 = marker_pos[i, 1]
            g0 = 0.0
            if vy < 0.0:
        ...
        if 0.0 <= delta_t <= 1.0:
            # The particle reaches the wall within this time step: perform a "slip"
            # (specular) elastic bounce - reverse the normal (y) velocity component
            # only, leaving the tangential (x, z) components unchanged.
            vy_new = -vy
            
            marker_pos[i, 0] += marker_vel[i, 0]
            marker_pos[i, 1] = y0 + vy * delta_t + vy_new * (1.0 - delta_t)
            marker_pos[i, 2] += marker_vel[i, 2]
            
            marker_vel[i, 1] = vy_new
        else:
            marker_pos[i, 0] += marker_vel[i, 0]
...
def run_diff_sim(pops_pre, pops_post, F, rho, u, u_mag_sq, N_markers, marker_pos, marker_vel, marker_f, marker_nh, marker_nh_size, 
                 Nt, F_brownian_scale, Nx, Ny, Nz, n_lattice, lubrication_threshold, r_cutoff_outer, r_cutoff_outer_sq, r_cutoff_inner_sq, dist_func, r_gaus, sigma, A, stopping_lims, 
    ...
    wall_y = stopping_lims[1][0]  # bottom wall position (particle-wall interactions)
    ...
    marker_radii = np.array(D_particles, dtype=np.float64)/2
    ...
    for t in iterations:
        ...
        if t<120:
            F_ini = 1.5
            marker_f[0, 0] = F_ini # initial force marker 1
            marker_f[0, 1] = F_ini
            marker_f[1, 0] = -F_ini # initial force marker 2
            marker_f[1, 1] = F_ini
        else:
            marker_f[0, 0] = 0
            marker_f[0, 1] = 0
        ...
        F_gravity = 0.75 # gravity force magnitude
        marker_f[:, 1] += -F_gravity # apply gravity in the -y direction
        
        # Calculate marker forces
        #lubrication_correction_forcing(N_markers, marker_pos, marker_vel, marker_radii, 
        #                    lubrication_threshold, marker_f, wall_y, F_gravity)
        ...
        # elastic chock
        resolve_particle_collisions(marker_pos, marker_vel, D_particles, rhos, wall_y)
    ...
    if live_flow_plot and len(frames) > 0:
        imageio.mimsave(f'{output_gif_name}.gif', frames, fps=15, loop=0)
