...
output_gif_name = "gravity_wall" # name of the output gif file (only if live_flow_plot=True)

# Geometry
D_particle1 = 7 # number of lattice points across the particle diameter
D_particles = [D_particle1] # list of particle diameters
...
cy_particle = (Ny-1)/4 # particle initial y position
...
def lubrication_correction_forcing(N_markers, marker_pos, marker_vel, marker_radii, 
    ...
    for i in range(N_markers):
        ...
        if h_wall <= h_min:
            ...
            marker_f[i, 1] = 0.2*((wall_y + marker_radii[i]) - marker_pos[i, 1])
            ...
            coef = -6 * np.pi * nu * R_eff**2 * (1/h_wall - 1/lubrication_threshold) * UR
...
def run_diff_sim(pops_pre, pops_post, F, rho, u, u_mag_sq, N_markers, marker_pos, marker_vel, marker_f, marker_nh, marker_nh_size, 
    ...
    for t in iterations:
        ...
        F_gravity = 0.4 # gravity force magnitude
