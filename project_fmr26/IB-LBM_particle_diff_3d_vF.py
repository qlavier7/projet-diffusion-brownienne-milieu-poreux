...
output_name = 'particle_diffusion_3d' # name of the output file name
...
rhos = np.ones(N_markers, dtype=np.float64) * rho_0 # list of particle densities, equal to the fluid density
...
collision_time_model = "normal" # "hertzian" or "normal"
...
def save_interactive_3d_html(filename, fluid_history, marker_pos_history,
    ...
    title_text = (
    fr"3D Particle Diffusion<br>"
    fr"$\rho_p = {rhos[0]/rho_0:.3g}\rho_0$<br>"
    fr"$\nu = {f_nu.numerator}/{f_nu.denominator}$<br>"
    fr"$k_B T = {kB_T:.3g}$<br>"
    fr"porosity = {1-volume_cylinders/volume_domain:.3g}"
    )
                      ...
                      steps=[dict(label=str(index), method='animate',
                                  args=[[str(index)], {'mode': 'immediate',
                                                       'frame': {'duration': 0,
                                                                 'redraw': True}}])
                             for index in range(len(fluid_speeds))])])
    figure.write_html(filename, include_plotlyjs=True, include_mathjax="cdn")
