...
pops_post = np.empty_like(pops_pre) # second DVDF array for efficient data writing during streamingS
...
def run_diff_sim(pops_pre, pops_post, F, rho, u, u_mag_sq, N_markers, marker_pos, marker_vel, marker_f, marker_nh, marker_nh_size, 
        ...
        if save_data and live_flow_plot:
            ...
            for c_idx in range(N_cylinders):
                ...
                if sorted_pts is not None and len(sorted_pts) > 3:
                    ...
                    polygon_outline = Polygon(sorted_pts, fill=False, edgecolor='maroon', linewidth=1.5, zorder=6)
    ...
    if live_flow_plot and len(frames) > 0:
        imageio.mimsave('particle_diffusion_v3.gif', frames, fps=5, loop=0)
